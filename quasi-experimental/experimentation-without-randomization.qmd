...
The thing you are hoping for is a flat pattern during the multiple evaluations before the intervention, a sudden jump right at the intervention, and a flat pattern for all the evaluations after the intervention. In contrast an upward trend at each and every evaluation is very strong negative evidence.

You might see other patterns. If a patient is getting worse over time, does the slope prior to the intervention level out somewhat after the intervention?

## Phased intervetions

It can help even more if your intervention one that can be split and implemented in different phases. If so, make multiple evaluations, implement one phase of the intervention, make additional evaluations, implement a second phase of the intervention, and so forth.

You are hoping for a flat-jump-flat-jump-flat pattern.
...
There is no easy answer to how many evaluation points you might need. If your data is noisy, you might need quite a few evaluation points. Even without a lot of noise, you want enough evaluation points to identify and rule out any cyclic patterns. For monthly evaluations, a couple of years would be nice so you can factor out seasonality. For daily evaluations, a couple of weeks so you can rule out any "hump day" effects.

The interesting issue is that you may be able to reduce the number of patients. The pattern observed in the single self-biting patient is quite revealing. This is an example of an "n of 1" design, where changes correspond precisely to the times when the intervention is added (or removed). 
